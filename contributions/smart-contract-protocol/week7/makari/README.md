...
A command-line tool or minimal web interface will allow users to: upload SBOMs, compute hashes, register them, verify files, and view version history. Smart contracts will be written in Solidity and deployed on an Ethereum Layer-2 testnet, with off-chain hashing handled via Python or Node.js.
